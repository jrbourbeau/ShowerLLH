    ...
    for i, (x, y) in enumerate(grid):
        ...
        for e in range(nE):
            ...
            for p in range(ntanks):
                llh += llhTable[idx[0], idx[1],
                                idx[2][p], idx[3][p], idx[4][p]]
...
""" Calculate array of LLH values for different energy bins
    ...
    Much faster than python, flexible version of code
 """


def getLLH_C(grid, llhTable, bins, binnedVals, theta, phi, tankxyz):

    # nDims = int(len(bins))
    nDims = sum([1 for i in bins if bins[i] is not None])
    binDims = np.array(
        [len(bins[i]) - 1 for i in bins if bins[i] is not None], dtype=np.int32)
    nG = int(len(grid))
    nE = int(len(bins['E']) - 1 )
    ntanks = int(len(tankxyz))
    ...
    argnames += ['binDims', 'nDims', 'nG', 'nE',
                 'ntanks', 'e_idx', 'd_idx', 'Dbins']
...
""" Calculate array of LLH values for different energy bins
    Uses weave (C) to calculate closest approach distances and llh
    Fastest version of code requires input of very specific format
 """


def getLLH_fast(grid, llhTable, bins, binnedVals, theta, phi, tankxyz):

    # ONLY WORKS ON 5-D hists:  ( E x Z x S x D x C )
    nG = int(len(grid))
    nE = int(len(llhTable))
    ntanks = int(len(tankxyz))
    RecoMaxLLHs = np.array([-np.inf for i in range(nG)], dtype=np.double)
    ...
    Dbins = np.asarray(bins[3][1], dtype=np.double)
    ...
    weave.inline(loopcode, ['nG', 'grid', 'theta', 'phi', 'ntanks', 'tankx', 'tanky', 'tankz', 'Dbins', 'nE',
                            'llhTable', 'RecoMaxLLHs', 'Zbin', 'Sbin', 'Dbin', 'Cbin', 'argmax'], type_converters=weave.converters.blitz)
...
def getLLH2(hists, varList, binList):
    ...
    loopcode = """
        ...
        return_val = 0;
        """
    weave.inline(loopcode, ['phist', 'fhist', 'pLLHs', 'fLLHs', 'Zbin', 'Sbin',
                            'Dbin', 'Cbin', 'tanks'], type_converters=weave.converters.blitz)
